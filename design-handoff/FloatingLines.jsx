// FloatingLines — fond animé WebGL (lignes/ondes) adapté pour OddWave.
// Three.js est fourni en global (window.THREE) via le CDN chargé dans le <helmet>.
// React est injecté globalement par le runtime DC.
...
float wave(vec2 uv, float offset, vec2 screenUv, vec2 mouseUv, bool shouldBend) {
  ...
  float m = uv.y - y;
  return 0.0042 / max(abs(m) + 0.003, 1e-3) + 0.0008;
}
...
function FloatingLines({
  ...
  useEffect(() => {
    ...
    if (!THREE) { console.warn('[FloatingLines] window.THREE introuvable'); return; }
    ...
    if (linesGradient && linesGradient.length > 0) {
      ...
      stops.forEach((hex, i) => {
        const color = hexToVec3(THREE, hex);
